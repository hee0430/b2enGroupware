...
(function($) {
    ...
    $.fn.DualListBox = function(paramOptions, selected) {
        return this.each(function () {
            ...
            for(var i = 0 ; i < selected.length ; i++){
            	...
            	if(selectedItem.intrfcId){
	            	 $('<option>', {
	            		 value: selectedItem.intrfcId
	            		 ,text: selectedItem.intrfcNm
	            	 }).appendTo('.selected');
            	}
            }

            //<option value="SJS_DB_001_복사2">SJS_DB_001_복사2</option>
    ...
    function addElementsViaJSON(options, selected) {
        ...
        $.each(options.dataList, function(key, item) {
            var text = '';
    ...
    function createDualListBox(options) {
        ...
        $(options.parentElement).addClass('row').append(
                (options.horizontal == false ? '   <div class="col-md-5" style="padding-right:0px;">' : '   <div class="col-md-6">') +
                '       <h5 style="color:#5B5B5B; font-weight:700;"><span class="unselected-title"></span> <small> <span class="unselected-count"></span></small></h5>' +
                '       <input class="filter form-control filter-unselected" type="text" placeholder="검색" style="margin-bottom: 5px;">' +
                (options.horizontal == false ? '' : createHorizontalButtons(1, options.moveAllBtn)) +
                '       <select class="unselected" style="font-size:13px; height: 300px; width: 100%;" multiple></select>' +
                '   </div>' +
                (options.horizontal == false ? createVerticalButtons(options.moveAllBtn) : '') +
                (options.horizontal == false ? '   <div class="col-md-5" style="padding-left:0px;">' : '   <div class="col-md-6">') +
                '       <h5 style="color:#5B5B5B; font-weight:700;"><span class="selected-title"></span> <small> <span class="selected-count"></span></small></h5>' +
                '       <input class="filter form-control filter-selected" type="text" placeholder="검색" style="margin-bottom: 5px;">' +
                (options.horizontal == false ? '' : createHorizontalButtons(2, options.moveAllBtn)) +
                '       <select class="selected" style="font-size:13px; height: 300px; width: 100%;" multiple></select>' +
                '   </div>');

        $(options.parentElement + ' .selected').prop('name', $(options.element).prop('name'));
        $(options.parentElement + ' .unselected-title').text('목록');
        $(options.parentElement + ' .selected-title').text('결과');
    }
    ...
    function createVerticalButtons(copyAllBtn) {
        return '   <div class="col-md-2 center-block" style="margin-top: ' + (copyAllBtn ? '120px' : '130px') +'">' +
            (copyAllBtn ? '       <button type="button" class="btn btn-white col-md-10 atr" data-type="atr" style="margin-left:10px;margin-bottom: 10px;"><span class="glyphicon glyphicon-list" style="padding-left:4px;"></span> <span class="glyphicon glyphicon-chevron-right"></span></button>' : '') +
            '       <button type="button" class="btn btn-white col-md-10 str" data-type="str" style="margin-left:10px;margin-bottom: 20px;" disabled><span class="glyphicon glyphicon-chevron-right" style="padding-left:12px;"></span></button>' +
            '       <button type="button" class="btn btn-white col-md-10 stl" data-type="stl" style="margin-left:10px;margin-bottom: 10px;" disabled><span class="glyphicon glyphicon-chevron-left" style="padding-left:12px;"></span></button>' +
            (copyAllBtn ? '       <button type="button" class="btn btn-white col-md-10 atl" data-type="atl" style="margin-left:10px;margin-bottom: 10px;"><span class="glyphicon glyphicon-chevron-left" style="padding-left:4px;"></span> <span class="glyphicon glyphicon-list"></span></button>' : '') +
            '   </div>';
    }
