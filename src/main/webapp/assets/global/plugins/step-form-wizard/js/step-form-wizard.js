...
(function($) {
    ...
    var defaults = {
        ...
        saveAndDistBtn: $('<input class="save-n-dist-btn sf-right sf-btn" type="submit" value="SAVE&DIST"/>'),
        ...
        onSaveAndDist: function(i, wizard) {},
    ...
    function stepFormWizard(element, options) {
        ...
        widget.btnsaveAndDistTmp = widget.config.saveAndDistBtn;
    ...
    stepFormWizard.prototype.init = function() {
        ...
        this.steps.each(function(index) {
            ...
            if(index == widget.config.startStep) {
                ...
                widget.initBtnSaveAndDist(widget.config.startStep);
                ...
                if(widget.config.startStep != widget.steps.length - 1) {
                    ...
                    widget.element.find('.sf-btn-save-n-dist').hide();
                } else {
        ...
        widget.element.on('click', '.save-n-dist-btn', function(e, data) {
            var ret = false;
            var event = jQuery.Event( 'sf-save-n-dist' );
            widget.element.trigger(event, [widget.stepActive, widget.stepActive, widget.wizard]);
            if(widget.config.onSaveAndDist(widget.stepActive, widget.wizard) === false || event.isDefaultPrevented()) {
                e.preventDefault();
            } else {
                ret = true;
            }
    ...
    stepFormWizard.prototype.goTo = function(index) {
        ...
        if(index + 1 >= this.steps.length) {
            ...
            this.btnSaveAndDist.data('step', index - 1).fadeIn(100);
        } else {
            this.btnFinish.fadeOut(0);
            this.btnSaveAndDist.fadeOut(0);
        }
        if(index == 0) {
    ...
    stepFormWizard.prototype.initBtnSaveAndDist = function(index) {
        $('.sf-step-' + index).append(" ");
        this.btnSaveAndDist = this.btnsaveAndDistTmp
            .clone(true)
            .addClass('sf-btn-save-n-dist')
            .data('step', index - 1);
        this.btnSaveAndDist.appendTo($(this.controls));
    }
    ...
    stepFormWizard.prototype.savendist = function() {
        var data = {ret: false};
        $('.sf-controls .save-n-dist-btn', this.element).trigger('click', [data]);
        return data.ret;
    }
