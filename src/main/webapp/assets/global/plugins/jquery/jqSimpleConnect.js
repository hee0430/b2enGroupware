 ...
 */
jqSimpleConnect._positionConnection = function(connection, color, borderStyle) {
    ...
    if(posA.left == posB.left || posA.top == posB.top) {
        ...
        if(posA.left == posB.left) {
            // Vertical line.
            jqSimpleConnect._positionVerticalLine(line1, posA, posB, connection.radius, connection.roundedCorners, color, borderStyle);
        } else {
            // Horizontal line.
            jqSimpleConnect._positionHorizontalLine(line1, posA, posB, connection.radius, connection.roundedCorners, color, borderStyle);
        }
    } else {
        ...
        if(connection.anchorA != connection.anchorB) {
            ...
            if(connection.anchorA == 'vertical') {
                ...
                // Draw lines.
                jqSimpleConnect._positionVerticalLine(line1, posA, corner, connection.radius, connection.roundedCorners, color, borderStyle);
                jqSimpleConnect._positionHorizontalLine(line2, posB, corner, connection.radius, connection.roundedCorners, color, borderStyle);
            } else {
                // Find the corner's position.
                ...
                // Draw lines.
                jqSimpleConnect._positionVerticalLine(line1, posB, corner, connection.radius, connection.roundedCorners, color, borderStyle);
                jqSimpleConnect._positionHorizontalLine(line2, posA, corner, connection.radius, connection.roundedCorners, color, borderStyle);
            }
        } else {
            // Use three lines.
            line1.show();
            ...
            if(connection.anchorA == 'vertical') {
                ...
                // Draw lines.
                jqSimpleConnect._positionVerticalLine(line1, posA, corner1, connection.radius, connection.roundedCorners, color, borderStyle);
                jqSimpleConnect._positionVerticalLine(line2, posB, corner2, connection.radius, connection.roundedCorners, color, borderStyle);
                jqSimpleConnect._positionHorizontalLine(line3, corner1, corner2, connection.radius, connection.roundedCorners, color, borderStyle);
            } else {
                ...
                // Draw lines.
                jqSimpleConnect._positionHorizontalLine(line1, posA, corner1, connection.radius, connection.roundedCorners, color, borderStyle);
                jqSimpleConnect._positionHorizontalLine(line2, posB, corner2, connection.radius, connection.roundedCorners, color, borderStyle);
                jqSimpleConnect._positionVerticalLine(line3, corner1, corner2, connection.radius, connection.roundedCorners, color, borderStyle);
            }
        }
    }
}
 ...
 */
jqSimpleConnect._positionVerticalLine = function(jqElement, point1, point2, radius, roundedCorners, color,  borderStyle) {
    ...
    if(borderStyle){
    	jqElement.css('border-left-style', borderStyle);
    	if('red' == color){
    		jqElement.css('color', 'white');
    	}else{
 ...
 *@param {boolean} roundedCorners A boolean indicating if the corners are going to be round.
 */
jqSimpleConnect._positionHorizontalLine = function(jqElement, point1, point2, radius, roundedCorners, color, borderStyle) {
    var halfRadius = parseInt(radius/2, 10);
    ...
    if(borderStyle){
    	jqElement.css('border-top-style', borderStyle);
    	if('red' == color){
    		jqElement.css('color', 'white');
    	}else{
    		jqElement.css('color', 'gray');
...
jqSimpleConnect.connect = function(elementA, elementB, options) {
    ...
    var div = '<div id="divUniqueIdentifier" class="jqSimpleConnect '+connection.id+'" ' +
              'style="z-index:2;width:'+connection.radius+'px; ' +
...
jqSimpleConnect.changeConnectLineStyle = function(connectionId, color, borderStyle) {
    var connection = jqSimpleConnect._connections[connectionId];
    if(connection != null) {
        jqSimpleConnect._positionConnection(connection, color, borderStyle);
        return true;
    }
    return false;
}
