...
jui.define("chart.draw", [ "util.base" ], function(_) {
	...
	var Draw = function() {
        ...
        /**
		 * @method format Get a default format callback of draw object.
		 *
		 * @return {Function}
		 */
    ...
    Draw.setup = function() {
        return {
            /**
			 * @cfg {String} [type=null] Specifies the type of a
			 *      widget/brush/grid to be added.
			 */
            type: null,
...
jui.define("chart.axis", [ "util.base" ], function(_) {
	 ...
	 *
	 * Axis 를 관리하는 클래스
	 *  * x 축 * y 축 * area { x, y, width, height} * data Axis 에 적용될 데이타
	 *
	 */
    var Axis = function(chart, originAxis, cloneAxis) {
		 ...
		 * Axis 의 표시 영역을 리턴한다.
		 *
		 * @param {"x"/"y"/"width"/'height"/null}
		 *            key area's key
		 * @return {Number/Object} key 가 있으면 해당 key 의 value 를 리턴한다. 없으면 전체 area
		 *         객체를 리턴한다.
		 */
		 ...
		 *
		 * @param {"x"/"y"/"c"/"map"/"degree"/"padding"}
		 *            type
		 * @param {Object}
		 *            grid
		 */
    ...
    Axis.setup = function() {
        ...
        /**
		 * @property {chart.grid.core} [x=null] Sets a grid on the X axis (see
		 *           the grid tab).
		 */
        /**
		 * @property {chart.grid.core} [y=null] Sets a grid on the Y axis (see
		 *           the grid tab).
		 */
        /**
		 * @property {chart.grid.core} [c=null] Sets a custom grid (see the grid
		 *           tab).
		 */
        /** @property {chart.map} [map=null] Sets a chart map. */
        /**
		 * @property {Array} [data=[]] Sets the row set data which constitute a
		 *           chart.
		 */
        /**
		 * @property {Integer} [buffer=10000] Limits the number of elements
		 *           shown on a chart.
		 */
        /**
		 * @property {Integer} [shift=1] Data shift count for the 'prev' or
		 *           'next' method of the chart builder.
		 */
        /**
		 * @property {Array} [origin=[]] [For read only] Original data initially
		 *           set.
		 */
        /**
		 * @property {Integer} [page=1] [For read only] Page number of the data
		 *           currently drawn.
		 */
        /**
		 * @property {Integer} [start=0] [For read only] Start index of the data
		 *           currently drawn.
		 */
        /**
		 * @property {Integer} [end=0] [For read only] End index of the data
		 *           currently drawn.
		 */

        return {
            /**
			 * @cfg {Integer} [extend=null] Configures the index of an
			 *      applicable grid group when intending to use already
			 *      configured axis options.
			 */
            extend: null,

            /**
			 * @cfg {chart.grid.core} [x=null] Sets a grid on the X axis (see
			 *      the grid tab).
			 */
            x: null,
            /**
			 * @cfg {chart.grid.core} [y=null] Sets a grid on the Y axis (see
			 *      the grid tab).
			 */
            y: null,
            /**
			 * @cfg {chart.grid.core} [z=null] Sets a grid on the Z axis (see
			 *      the grid tab).
			 */
            z: null,
            /**
			 * @cfg {chart.grid.core} [c=null] Sets a grid on the C axis (see
			 *      the grid tab).
			 */
            c: null,
            ...
            map : null,
            /**
			 * @cfg {Array} [data=[]] Sets the row set data which constitute a
			 *      chart.
			 */
            data: [],
            /**
			 * @cfg {Array} [origin=[]] [Fore read only] Original data initially
			 *      set.
			 */
            origin: [],
            ...
            /**
			 * @cfg {Number} [buffer=10000] Limits the number of elements shown
			 *      on a chart.
			 */
            buffer: 10000,
            /**
			 * @cfg {Number} [shift=1] Data shift count for the 'prev' or 'next'
			 *      method of the chart builder.
			 */
            shift: 1,
            ...
            depth: 0,
            /**
			 * @cfg {Number} [perspective=0.9] Set perspective values in the 3d
			 *      chart
			 */
            perspective: 0.9
        }
...
jui.define("chart.map", [ "util.base", "util.dom", "util.math", "util.svg" ], function(_, $, math, SVG) {
    ...
    Map.setup = function() {
        ...
        return {
            ...
            /**
			 * @cfg {Boolean} [hide=false] Determines whether to display an
			 *      applicable grid.
			 */
            hide: false,
    ...
    /**
	 * @event map_click Event that occurs when clicking on the map area. (real
	 *        name ``` map.click ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 ...
	 */
    /**
	 * @event map_dblclick Event that occurs when double clicking on the map
	 *        area. (real name ``` map.dblclick ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
	 */
    /**
	 * @event map_rclick Event that occurs when right clicking on the map area.
	 *        (real name ``` map.rclick ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
	 */
    /**
	 * @event map_mouseover Event that occurs when placing the mouse over the
	 *        map area. (real name ``` map.mouseover ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
	 */
    /**
	 * @event map_mouseout Event that occurs when moving the mouse out of the
	 *        map area. (real name ``` map.mouseout ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
	 */
    /**
	 * @event map_mousemove Event that occurs when moving the mouse over the map
	 *        area. (real name ``` map.mousemove ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
	 */
    /**
	 * @event map_mousedown Event that occurs when left clicking on the map
	 *        area. (real name ``` map.mousedown ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
	 */
    /**
	 * @event map_mouseup Event that occurs after left clicking on the map area.
	 *        (real name ``` map.mouseup ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 * @param {Number}
	 *            index Axis index.
...
jui.defineUI("chart.builder", [ "util.base", "util.dom", "util.svg", "util.color", "chart.axis" ],
    ...
    var UI = function() {
        ...
        var _initialize = false, _options = null, _handler = { render: [], renderAll: [] }; // 리셋
																							// 대상
																							// 커스텀
																							// 이벤트
																							// 핸들러
		 ...
		 * @method get
		 *
		 * Gets a named axis, brush, widget (type: axis, brush, widget, padding,
		 * area)
		 *
		 * @param {"axis"/"brush"/"widget"/"padding"/"area"}
		 *            type
		 * @param {String}
		 *            key Property name
		 * @return {Mixed/Object}
        ...
        /**
		 * Gets a calculated value for a chart area (type: width, height, x, y,
		 * x2, y2)).
		 *
		 * @param {String}
		 *            key
		 ...
		 *
		 * ``` // get all theme property var theme = chart.theme(); // get a
		 * part of theme var fontColor = chart.theme("fontColor"); // get
		 * selected value of theme chart.theme(isSelected, "selectedFontColor",
		 * "fontColor"); // if isSelected is true, return 'selectedFontColor'
        ...
        /**
		 * Returns a value from the format callback function of a defined
		 * option.
		 *
		 * @param {Function}
		 *            format
		 ...
		 * @method on
		 *
		 * A callback function defined as an on method is run when an emit
		 * method is called.
		 *
		 * @param {String}
		 *            type Event's name
		 * @param {Function}
		 *            callback
		 * @param {"render"/"renderAll"/undefined}
		 *            resetType
		 */
        ...
        /**
		 * @method updateBrush Updates the brush of a specified index and
		 *         performs rendering again.
		 * @param {Number}
		 *            index
		 * @param {Object}
		 *            brush
        ...
        /**
		 * @method addWidget Adds a widget and performs rendering again.
		 *
		 * @param {Object}
		 *            widget
		 */
        ...
        /**
		 * @method removeWidget Deletes the widget of a specified index and
		 *         performs rendering again.
		 * @param {Number}
		 *            index
		 */
        ...
        /**
		 * @method updateWidget Updates the widget of a specified index and
		 *         performs rendering again
		 * @param {Number}
		 *            index
		 * @param {Object}
        ...
        /**
		 * Changes the size of a chart to the specified area and height then
		 * performs rendering.
		 *
		 * @param {Number}
		 *            width
        ...
        /**
		 * Returns the values of rendering options and, if the rendering option
		 * is false, does not render the chart again when a method is called.
		 *
		 * @return {Boolean}
		 */
    ...
    UI.setup = function() {
        return {
            ...
            bind: null,
            /**
			 * @cfg {Function} [format=null] Sets a format callback function to
			 *      be used in a grid/brush/widget.
			 */
            format: null,
            /**
			 * @cfg {Boolean} [render=true] Does not render a chart when a
			 *      rendering-related method is called with false (although the
			 *      render method is not included).
			 */
            render: true,
    ...
    /**
	 * @event chart_click Event that occurs when clicking on the chart area.
	 *        (real name ``` chart.click ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_dblclick Event that occurs when double clicking on the chart
	 *        area. (real name ``` chart.dblclick ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_rclick Event that occurs when right clicking on the chart
	 *        area. (real name ``` chart.rclick ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_mouseover Event that occurs when placing the mouse over the
	 *        chart area. (real name ``` chart.mouseover ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_mouseout Event that occurs when moving the mouse out of the
	 *        chart area. (real name ``` chart.mouseout ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_mousemove Event that occurs when moving the mouse over the
	 *        chart area. (real name ``` chart.mousemove ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_mousedown Event that occurs when left clicking on the chart
	 *        area. (real name ``` chart.mousedown ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event chart_mouseup Event that occurs after left clicking on the chart
	 *        area. (real name ``` chart.mouseup ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */

    /**
	 * @event bg_click Event that occurs when clicking on the chart margin.
	 *        (real name ``` bg.click ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_dblclick Event that occurs when double clicking on the chart
	 *        margin. (real name ``` bg.dblclick ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_rclick Event that occurs when right clicking on the chart
	 *        margin. (real name ``` bg.rclick ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_mouseover Event that occurs when placing the mouse over the
	 *        chart margin. (real name ``` bg.mouseover ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_mouseout Event that occurs when moving the mouse out of the
	 *        chart margin. (real name ``` bg.mouseout ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_mousemove Event that occurs when moving the mouse over the
	 *        chart margin. (real name ``` bg.mousemove ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_mousedown Event that occurs when left clicking on the chart
	 *        margin. (real name ``` bg.mousedown ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
    /**
	 * @event bg_mouseup Event that occurs after left clicking on the chart
	 *        margin. (real name ``` bg.mouseup ```)
	 * @param {jQueryEvent}
	 *            e The event object.
	 */
...
jui.define("chart.theme.jennifer", [], function() {
    ...
    /**
	 * @class chart.theme.jennifer Jennifer Theme
	 * @singleton
	 */
    var themeColors = [
        "#7977C2",
    ...
    return {
        ...
        topologyActiveEdgeColor : "#EAFF32",//"#905ed1",
        topologyHoverEdgeColor : "#EAFF32",//"#d3bdeb",
...
jui.define("chart.grid.draw2d", [ "util.base", "util.math" ], function(_, math) {
    ...
    var Draw2DGrid = function() {
        ...
        /**
		 * @method drawAxisLine theme 이 적용된 axis line 리턴
		 * @param {ChartBuilder}
		 *            chart
		 * @param {Object}
		 *            attr
...
jui.define("chart.grid.draw3d", [ "util.base", "chart.polygon.grid", "chart.polygon.line", "chart.polygon.point" ],
    ...
    var Draw3DGrid = function() {
        ...
        /**
		 * @method axisLine theme 이 적용된 axis line 리턴
		 * @param {ChartBuilder}
		 *            chart
		 * @param {Object}
		 *            attr
...
jui.define("chart.grid.core", [ "util.base", "util.math", "chart.grid.draw2d", "chart.grid.draw3d" ],
	...
	/**
	 * @class chart.grid.core Grid Core 객체
	 * @extends chart.draw
	 * @abstract
	 */
	var CoreGrid = function() {
		...
		/**
		 * @method wrapper scale wrapper
		 *
		 * grid 의 x 좌표 값을 같은 형태로 가지고 오기 위한 wrapper 함수
		 *
		 * grid 속성에 key 가 있다면 key 의 속성값으로 실제 값을 처리
		 *
		 * @example // 그리드 속성에 키가 없을 때 scale(0); // 0 인덱스에 대한 값 (block, radar) //
		 *          grid 속성에 key 가 있을 때 grid { key : "field" } scale(0) // field
		 *          값으로 scale 설정 (range, date)
		 *
		 * @protected
		 */
		...
		/**
		 * @method line theme 이 적용된 line 리턴
		 * @protected
		 * @param {ChartBuilder}
		 *            chart
		 * @param {Object}
		...
		/**
		 * @method color grid 에서 color 를 위한 유틸리티 함수
		 * @param theme
		 * @return {Mixed}
		 */
		...
		/**
		 * @method getTextRotate implement text rotate in grid text
		 * @protected
		 * @param {SVGElement}
		 *            textElement
		 */
		...
		/**
		 * @method drawGrid draw base grid structure
		 * @protected
		 * @param {chart.builder}
		 *            chart
		 * @param {String}
	...
	CoreGrid.setup = function() {
		...
		return {
			...
			dist: 0,
			/**
			 * @cfg {"top"/"left"/"bottom"/"right"} [orient=null] Specifies the
			 *      direction in which an axis is shown (top, bottom, left or
			 *      right).
			 */
			orient: null,
			/**
			...
			hide: false,
			/**
			 * @cfg {String/Object/Number} [color=null] Specifies the color of a
			 *      grid.
			 */
			color: null,
			...
			title: null,
			/**
			 * @cfg {Boolean} [hide=false] Determines whether to display a line
			 *      on the axis background.
			 */
			line: false,
			/**
			 * @cfg {Function} [format=null] Determines whether to format the
			 *      value on an axis.
			 */
			format: null,
			/**
			 * @cfg {Function} [image=null] Determines whether to image the
			 *      value on an axis.
			 */
			image: null,
			/**
			 * @cfg {Number} [textRotate=null] Specifies the slope of text
			 *      displayed on a grid.
			 */
			textRotate : null
		};
...
jui.define("chart.grid.block", [ "util.scale", "util.base" ], function(UtilScale, _) {
	...
	/**
	 * @class chart.grid.block Implements Block Grid
	...
	var BlockGrid = function() {
		...
		this.initDomain = function() {
			...
			} else if (_.typeCheck("function", this.grid.domain)) {	// block 은
																	// 배열을 통째로
																	// 리턴함
	...
	BlockGrid.setup = function() {
		return {
			/**
			 * @cfg {String/Array/Function} [domain=null] Sets the value
			 *      displayed on an axis.
			 */
			domain: null,
			/**
			 * @cfg {Boolean} [reverse=false] Reverses the value on domain
			 *      values
			 */
			reverse: false,
			...
			max: 10,
			/**
			 * @cfg {Boolean} [hideText=false] Determines whether to show text
			 *      across the grid.
			 */
			hideText: false,
			/**
			 * @cfg {String} [key=null] Sets the value on the grid to the value
...
jui.define("chart.grid.date", [ "util.time", "util.scale", "util.base" ], function(UtilTime, UtilScale, _) {
	...
	var DateGrid = function() {
		...
		this.drawBefore = function() {
			...
			/*
			 * data 없을 때도 기본 설정만으로 보여야 하기 때문에. 지우겠음 if (this.axis.data.length ==
			 * 0) { //this.ticks = []; }
			 */
	...
	DateGrid.setup = function() {
		return {
			...
			domain: null,
			/**
			 * @cfg {Number} [interval=1000] Sets the interval of the scale
			 *      displayed on a grid.
			 */
			interval : 1000,
			...
			key: null,
			/**
			 * @cfg {"years"/"months"/"days"/"hours"/"minutes"/"seconds"/"milliseconds"}
			 *      [realtime=""] Determines whether to use as a real-time grid.
			 */
			realtime: null,
			/**
			 * @cfg {Boolean} [hideText=false] Determines whether to show text
...
jui.define("chart.grid.radar", [ "util.math", "util.base" ], function(math, _) {
	...
	RadarGrid.setup = function() {
		return {
			...
			max: 100,
			/**
			 * @cfg {Array} [step=10] Sets the interval of the scale displayed
			 *      on a grid.
			 */
            step : 10,
			/**
			 * @cfg {Boolean} [line=true] Determines whether to display a line
			 *      on the axis background.
			 */
			line: true,
			/**
			...
			hideText: false,
			/**
			 * @cfg {Boolean} [extra=false] Leaves a certain spacing distance
			 *      from the grid start point and displays a line where the
			 *      spacing ends.
			 */
			extra: false,
			/**
			 * @cfg {"radial"/"circle"} [shape="radial"] Determines the shape of
			 *      a grid (radial, circle).
			 */
...
jui.define("chart.grid.range", [ "util.scale", "util.base", "util.math" ], function(UtilScale, _, math) {
	...
	var RangeGrid = function() {
		...
		this.initDomain = function() {
			...
			if (_.typeCheck("function", this.grid.unit)) {
				...
				unit = math.div((max - min), this.grid.step);   // (max - min) /
																// this.grid.step
	...
	RangeGrid.setup = function() {
		return {
			...
			max: 0,
			/**
			 * @cfg {Number} [unit=null] Multiplies the axis value to be
			 *      displayed.
			 */
			unit: null,
			/**
			 ...
			 *
			 * max 나 min 을 넘어가는 값에 대한 체크, true 이면 넘어가는 값도 min, max 에서 조정, false
			 * 이면 비율로 계산해서 넘어가는 값 적용
			 */
			clamp : true,
			/**
			 * @cfg {Boolean} [reverse=false] Reverses the value on domain
			...
			hideText: false,
			/**
			 * @cfg {Boolean} [nice=false] Automatically sets the value of a
			 *      specific section.
			 */
			nice: false
		};
...
jui.define("chart.grid.rule", [ "util.scale", "util.base" ], function(UtilScale, _) {
	...
	var RuleGrid = function() {
        ...
        this.initDomain = function() {
            ...
            if (unit == 0) {
                ...
                domain = [end, start];
                // this.grid.step = Math.abs(start / unit) + Math.abs(end /
				// unit);
            }
	...
	RuleGrid.setup = function() {
		return {
			...
			hideText: false,
			/**
			 * @cfg {Boolean} [hideZero=false] Determines whether to show '0'
			...
			nice: false,
			/**
			 * @cfg {Boolean} [center=false] Place the reference axis in the
			 *      middle.
			 */
			center: false
...
jui.define("chart.topology.sort.linear", [], function() {
    ...
    return function(data, area, space) {
    	...
    	// TODO 위치를 저장했다가 불러오려면 여기가 적당해보임
    	var agentDataLength = 0;
        ...
        var xy = [],
            ...
            col_cnt = Math.floor((area.width-100) / space), // hhlee 마지막 노드가 영역
															// 밖으로 나가는거 막음
            col_step = Math.floor(col_cnt / agentDataLength),
            col_index = 0;
        ...
        var column = Math.floor((area.width-100) / agentDataLength);
        var agentIndex = 0;
        for(var i = 0; i < data.length; i++) {
            var x = 0, y = 0;
            var item = data[i];
			...
			if(item.type != 'agent'){
            	if(group[item.group] != undefined){
            		tmpXy = group[item.group];

            		var g = getRandomRowIndex(4);
            		var addValue1 = getRandomRowIndex(150);
            		var addValue2 = getRandomRowIndex(150);
            		if(addValue1 < 50){
            			addValue1 = 50;
            		}
            		if(addValue2 < 50){
            		...
            		if(g == 0){
            			tx = tmpXy.x + 50 + addValue1;
            			ty = tmpXy.y + 50 + addValue2;
            		}else if(g == 1){
            			tx = tmpXy.x - 50- addValue1;
            			ty = tmpXy.y + 50 + addValue2;
            		}else if(g == 2){
	...
	function getGroupData(data, key) {
	    for(var i = 0; i < data.length; i++) {
	        var d = data[i];
	        var k = self.getValue(d, "name");
...
jui.define("chart.brush.core", [ "util.base", "util.dom" ], function(_, $) {
	...
	var CoreBrush = function() {
        ...
        this.drawAfter = function(obj) {
            ...
            obj.translate(this.chart.area("x"), this.chart.area("y")); // 브러쉬일
																		// 경우,
																		// 기본 좌표
																		// 설정
        }
        ...
        /**
		 * @method addEvent 브러쉬 엘리먼트에 대한 공통 이벤트 정의
		 *
		 * @param {Element}
		 *            element
		 * @param {Integer}
        ...
        this.addEvent = function(elem, dataIndex, targetIndex) {
            ...
            function setMouseEvent(e) {
				...
				if(e.type == "mousemove" && isMouseClick){
					isMouseMove = true;
				}

                if(e.type == "mouseup" && isMouseClick && isMouseMove){
                	chart.emit("topology.dragstop", [ obj, e ]);
                	isMouseClick = false;
					isMouseMove = false;
                }
		 ...
		 *
		 * @param {Number}
		 *            key1 브러쉬에서 사용될 컬러 Index
		 * @param {Number}
		 *            key2 브러쉬에서 사용될 컬러 Index
		 * @returns {*}
		 */
    ...
    CoreBrush.setup = function() {
        return {
            ...
            /**
			 * @cfg {Array} [target=null] Specifies the key value of data
			 *      displayed on a brush.
			 */
            target: null,
            /**
			 * @cfg {Array/Function} [colors=null] Able to specify color codes
			 *      according to the target order (basically, refers to the
			 *      color codes of a theme)
			 */
            colors: null,
            /**
			 * @cfg {Integer} [axis=0] Specifies the index of a grid group which
			 *      acts as the reference axis of a brush.
			 */
            axis: 0,
            /**
			 * @cfg {Integer} [index=null] [Read Only] Sequence index on which
			 *      brush is drawn.
			 */
            index: null,
            /**
			 * @cfg {boolean} [clip=true] If the brush is drawn outside of the
			 *      chart, cut the area.
			 */
            clip: true,
            /**
			 * @cfg {boolean} [useEvent=true] If you do not use a brush events,
    ...
    /**
	 * @event click Event that occurs when clicking on the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event dblclick Event that occurs when double clicking on the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event rclick Event that occurs when right clicking on the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event mouseover Event that occurs when placing the mouse over the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event mouseout Event that occurs when moving the mouse out of the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event mousemove Event that occurs when moving the mouse over the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event mousedown Event that occurs when left clicking on the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
    /**
	 * @event mouseup Event that occurs after left clicking on the brush.
	 * @param {BrushData}
	 *            obj Related brush data.
	 */
...
jui.define("chart.brush.bar", [ "util.base" ], function(_) {
	...
	BarBrush.setup = function() {
		return {
			...
			size: 0,
            /**
			 * @cfg {Number} [minSize=0] Sets the minimum size as it is not
			 *      possible to draw a bar when the value is 0.
			 */
            minSize: 0,
            /**
			 * @cfg {Number} [outerPadding=2] Determines the outer margin of a
			 *      bar.
			 */
			outerPadding: 2,
            /**
			 * @cfg {Number} [innerPadding=1] Determines the inner margin of a
			 *      bar.
			 */
			innerPadding: 1,
            /**
			 * @cfg {Number} [active=null] Activates the bar of an applicable
			 *      index.
			 */
			active: null,
            /**
			 * @cfg {String} [activeEvent=null] Activates the bar in question
			 *      when a configured event occurs (click, mouseover, etc).
			 */
			activeEvent: null,
            /**
			 * @cfg {"max"/"min"/"all"} [display=null] Shows a tool tip on the
			 *      bar for the minimum/maximum value.
			 */
			display: null,
            /**
			 * @cfg {Function} [format=null] Sets the format of the value that
			 *      is displayed on the tool tip.
			 */
			format: null
		};
	}
...
jui.define("chart.brush.circle", ["util.base"], function(_) {
    ...
    CircleBrush.setup = function() {
        return {
            /**
			 * @cfg {Boolean} [clip=false] If the brush is drawn outside of the
			 *      chart, cut the area.
			 */
            clip: false,
...
jui.define("chart.brush.stackbar", [ "util.base" ], function(_) {
	...
	StackBarBrush.setup = function() {
		return {
			/**
			 * @cfg {Number} [outerPadding=15] Determines the outer margin of a
			 *      stack bar.
			 */
			outerPadding: 15,
...
jui.define("chart.brush.fullstackbar", [], function() {
	...
	FullStackBarBrush.setup = function() {
		return {
			...
			outerPadding: 15,
            /**
			 * @cfg {Boolean} [showText=false] Configures settings to let the
			 *      percent text of a full stack bar revealed.
			 */
			showText: false
		};
	}
...
jui.define("chart.brush.bubble", [ "util.base", "util.math" ], function(_, math) {
    ...
    BubbleBrush.setup = function() {
        return {
            ...
            max: 30,
            /**
			 * @cfg {String} [scaleKey=null] The name of the property to
            ...
            showText: false,
            /**
			 * @cfg {Function} [format=null] Returns a value from the format
			 *      callback function of a defined option.
			 */
            format: null,
            /**
			 * @cfg {Number} [active=null] Activates the bar of an applicable
...
jui.define("chart.brush.equalizer", [], function() {
    ...
    EqualizerBrush.setup = function() {
        return {
            /**
			 * @cfg {Number} [innerPadding=10] Determines the inner margin of an
			 *      equalizer.
			 */
            innerPadding: 10,
            /**
			 * @cfg {Number} [outerPadding=15] Determines the outer margin of an
			 *      equalizer.
			 */
            outerPadding: 15,
            /**
			 * @cfg {Number} [unit=5] Determines the reference value that
			 *      represents the color.
			 */
            unit: 5,
            /**
			 * @cfg {Number} [gap=5] Determines the number of columns in an
			 *      equalizer - expressed as a color.
			 */
            gap: 5
        };
    }
...
jui.define("chart.brush.line", [ "util.base" ], function(_) {
    ...
    LineBrush.setup = function() {
        return {
            /**
			 * @cfg {"normal"/"curve"/"step"} [symbol="normal"] Sets the shape
			 *      of a line (normal, curve, step).
			 */
...
jui.define("chart.brush.pie", [ "util.base", "util.math", "util.color" ], function(_, math, ColorUtil) {
    ...
    var PieBrush = function() {
        ...
        this.drawPie = function(centerX, centerY, outerRadius, startAngle, endAngle, color) {
            ...
            if (endAngle == 360) { // if pie is full size, draw a circle as pie
									// brush
    ...
    PieBrush.setup = function() {
        return {
            ...
            clip: false,
            /**
			 * @cfg {String} [showText=null] Set the text appear. (outside or
			 *      inside)
			 */
            showText: null,
            /**
            ...
            "3d": false,
            /**
			 * @cfg {String|Array} [active=null] Activates the pie of an
			 *      applicable property's name.
			 */
            active: null,
            /**
			 * @cfg {String} [activeEvent=null] Activates the pie in question
			 *      when a configured event occurs (click, mouseover, etc).
			 */
            activeEvent: null
        }
...
jui.define("chart.brush.donut", [ "util.base", "util.math", "util.color" ], function(_, math, ColorUtil) {
	...
	var DonutBrush = function() {
		...
		this.drawDonut = function(centerX, centerY, innerRadius, outerRadius, startAngle, endAngle, attr) {
            ...
            if (endAngle >= 360) { // bugfix : if angle is 360 , donut cang't
									// show
                endAngle = 359.9999;
            }
			...
			// 중심점 이동
			g.translate(centerX + ((this.brush.dx != 0) ? (this.brush.dx) : 0), centerY);
        ...
        this.drawUnit = function (index, data, g) {
			...
			if(target.length == 0){ // hhlee
				this.drawNoData(g);
			}
            // Show total value
            if(this.brush.showValue) {
                this.drawTotalValue(g, centerX + ((this.brush.dx != 0) ? (this.brush.dx) : 0), centerY, totalValue);
            }
        }
        ...
        this.drawTotalValue = function(g, centerX, centerY, value) {
            ...
            // var text = this.chart.text(config, this.format(value));
            var text = this.chart.text(config, value);
...
jui.define("chart.brush.scatter", [ "util.base" ], function(_) {
    ...
    ScatterBrush.setup = function() {
        return {
            /**
			 * @cfg {"circle"/"triangle"/"rectangle"/"cross"/"callback"}
			 *      [symbol="circle"] Determines the shape of a (circle,
			 *      rectangle, cross, triangle).
			 */
            symbol: "circle",
            /** @cfg {Number} [size=7] Determines the size of a starter. */
            size: 7,
            /**
			 * @cfg {Boolean} [hide=false] Hide the scatter, will be displayed
			 *      only when the mouse is over.
			 */
            hide: false,
            /**
			 * @cfg {Boolean} [hideZero=false] When scatter value is zero, will
			 *      be hidden.
			 */
            hideZero: false,
            /**
			 * @cfg {Boolean} [hoverSync=false] Over effect synchronization of
			 *      all the target's symbol.
			 */
            hoverSync: false,
            /**
			 * @cfg {String} [activeEvent=null] Activates the scatter in
			 *      question when a configured event occurs (click, mouseover,
			 *      etc).
			 */
            activeEvent: null,
            /**
			 * @cfg {"max"/"min"/"all"} [display=null] Shows a tooltip on the
			 *      scatter for the minimum/maximum value.
			 */
            display: null,
...
jui.define("chart.brush.scatterpath", ["util.base"], function(_) {
    ...
    ScatterPathBrush.setup = function() {
        return {
            /**
			 * @cfg {"circle"/"triangle"/"rectangle"/"cross"} [symbol="circle"]
			 *      Determines the shape of a starter (circle, rectangle, cross,
			 *      triangle).
			 */
            ...
            size: 7,
            /**
			 * @cfg {Number} [strokeWidth=1] Set the line thickness of a
			 *      starter.
			 */
            strokeWidth : 1
        };
...
jui.define("chart.brush.fillgauge", [ "util.base" ], function(_) {
	...
	var FillGaugeBrush = function(chart, axis, brush) {
		...
		this.draw = function() {
            ...
            if (brush.svg != "" || brush.path != "") {
                if (brush.svg != "") {
                    /***********************************************************
					 * / $.ajax({ url : brush.svg, async : false, success :
					 * function(xml) { var path = $(xml).find("path").attr("d");
    ...
    FillGaugeBrush.setup = function() {
        return {
            /**
			 * @cfg {Number} [min=0] Determines the minimum size of a fill
			 *      gauge.
			 */
            min: 0,
            /**
			 * @cfg {Number} [max=100] Determines the maximum size of a fill
			 *      gauge.
			 */
            max: 100,
            ...
            value: 0,
            /**
			 * @cfg {String} [shape="circle"] Determines the shape of a fill
			 *      gauge (circle, rectangle).
			 */
            shape: "circle", // or rectangle
            /**
			 * @cfg {String} [direction="vertical"] Determines the direction in
			 *      which a fill gauge is to be filled (vertical, horizontal).
			 */
            direction: "vertical",
            /**
			 * @cfg {String} [svg=""] Sets the shape of a fill gauge with a
			 *      specified URL as an SVG tag.
			 */
            svg: "",
            /**
			 * @cfg {String} [path=""] Sets the shape of a fill gauge with a
			 *      specified pass tag.
			 */
            path: ""
        };
...
jui.define("chart.brush.area", [ "util.base" ], function(_) {
    ...
    AreaBrush.setup = function() {
        return {
            ...
            activeEvent: null,
            /**
			 * @cfg {"max"/"min"} [display=null] Shows a tool tip on the bar for
			 *      the minimum/maximum value.
			 */
            display: null,
            /**
			 * @cfg {Boolean} [startZero=true] The end of the area is zero
			 *      point.
			 */
            startZero: true,
...
jui.define("chart.brush.gauge", [ "util.math" ], function(math) {
	...
	GaugeBrush.setup = function() {
		return {
			...
			size: 30,
            /**
			 * @cfg {Number} [startAngle=0] Determines the start angle(as start
			 *      point) of a gauge.
			 */
			startAngle: 0,
            /**
			 * @cfg {Number} [endAngle=360] Determines the end angle(as draw
			 *      point) of a gauge.
			 */
			endAngle: 360
		};
...
jui.define("chart.brush.stackgauge", [ "util.math" ], function(math) {
	...
	StackGaugeBrush.setup = function() {
		return {
			/**
			 * @cfg {Number} [min=0] Determines the minimum value of a stack
			 *      gauge.
			 */
			min: 0,
			/**
			 * @cfg {Number} [max=100] Determines the maximum value of a stack
			 *      gauge.
			 */
			max: 100,
			/**
			 * @cfg {Number} [cut=5] Determines the bar spacing of a stack
			 *      gauge.
			 */
			cut: 5,
			...
			size: 24,
			/**
			 * @cfg {Number} [startAngle=-180] Determines the start angle of a
			 *      stack gauge.
			 */
			startAngle: -180,
			/**
			 * @cfg {Number} [endAngle=360] Determines the end angle of a stack
			 *      gauge.
			 */
			endAngle: 360,
			/**
			 * @cfg {String} [title="title"] Sets a data key to be configured as
			 *      the title of a stack gauge.
			 */
			title: "title"
		};
	}
...
jui.define("chart.brush.waterfall", [], function() {
	...
	WaterFallBrush.setup = function() {
		return {
			/**
			 * @cfg {Boolean} [line=true] Connects with a line between columns
			 *      of a waterfall.
			 */
			line: true,
			/**
			 * @cfg {Boolean} [end=false] Sets effects for the last column of a
			 *      waterfall.
			 */
			end: false,
			/**
			 * @cfg {Boolean} [outerPadding=5] Determines the outer margin of a
			 *      waterfall.
			 */
			outerPadding: 5
		};
...
jui.define("chart.brush.splitline", [ "util.base" ], function(_) {
    ...
    SplitLineBrush.setup = function() {
        return {
            ...
            symbol: "normal", // normal, curve, step
            /**
			 * @cfg {Number} [split=null] Sets the style of a line of a
...
jui.define("chart.brush.topologynode",
    ...
    var TopologyNode = function() {
        ...
        function createNodes(index, data) {
            ...
            node.on(self.brush.activeEvent, function(e) {
                onNodeActiveHandler(data); // hhlee
        ...
        function createEdgeLine(edge, in_xy, out_xy) {
            ...
            var edgeData = getEdgeData(edge.key());
            var color = self.chart.theme("topologyEdgeColor");

            if(edgeData != null && edgeData.hasOwnProperty('color')){
            ...
            if(!edge.connect()) {
                ...
                reverseElem.get(0).attr({ "stroke-opacity": opacity, stroke:color });
            ...
            g.on(self.brush.activeEvent, function(e) {
                // onEdgeActiveHandler(edge); // hhlee
            });
        ...
        function createEdgeText(edge, in_xy, out_xy, line) {
            var text = null;
        ...
        /*
		 * this.eachData(function(data, i) { for(var j = 0; j <
		 * data.outgoing.length; j++) { setDataEdges(i, j); } });
		 */
            ...
            this.on("axis.mousedown", function(e) {
                if(self.axis.root.element == e.target) {
                    // onEdgeActiveHandler(null); // hhlee
                    tooltip.attr({ visibility: "hidden" });
                }
            });
    ...
    /**
	 * @event topoloygy_nodeclick Event that occurs when click on the topology
	 *        node. (real name ``` topoloygy.nodeclick ```)
	 * @param {Object}
	 *            data The node data.
	 * @param {jQueryEvent}
	 *            e The event object.
    ...
    /**
	 * @event topoloygy_edgeclick Event that occurs when click on the topology
	 *        edge. (real name ``` topoloygy.edgeclick ```)
	 * @param {Object}
	 *            data The edge data.
	 * @param {jQueryEvent}
	 *            e The event object.
...
jui.define("chart.brush.pin", [ "util.base" ], function(_) {
    ...
    PinBrush.setup = function() {
        return {
            ...
            size: 6,
            /**
			 * @cfg {Number} [split=0] Determines a location where a pin is
			 *      displayed (data index).
			 */
            split: 0,
            /** @cfg {Function} [format=null] */
            format: null,
...
jui.define("chart.brush.treemap.node", [], function() {
    ...
    var Node = function(data) {
        ...
        /**
		 * @property {Integer} [nodenum=null] Unique number of a specifiede node
		 *           at the current depth
		 */
        this.nodenum = null;

        /**
		 * @property {ui.tree.node} [parent=null] Variable that refers to the
		 *           parent of the current node
		 */
        this.parent = null;

        /**
		 * @property {Array} [children=null] List of child nodes of a specified
		 *           node
		 */
        this.children = [];
...
jui.define("chart.brush.treemap.nodemanager", [ "util.base", "chart.brush.treemap.node" ], function(_, Node) {
   ...
   var NodeManager = function() {
       ...
       this.getNodeParent = function(index) { // 해당 인덱스의 부모 노드를 가져옴 (단, 해당
												// 인덱스의 노드가 없을 경우)
           var keys = iParser.getIndexList(index);
...
jui.define("chart.brush.treemap.container", [ "util.treemap" ], function(util) {
    ...
    var Container = function(xoffset, yoffset, width, height) {
        ...
        this.getCoordinates = function (row) {
            var coordinates = [],
                subxoffset = this.xoffset, subyoffset = this.yoffset, // our
																		// offset
																		// within
																		// the
																		// container
        ...
        // cutArea - once we've placed some boxes into an row we then need to
		// identify the remaining area,
        // this function takes the area of the boxes we've placed and calculates
		// the location and
        // dimensions of the remaining space and returns a container box defined
		// by the remaining area
...
jui.define("chart.brush.treemap.calculator", [ "util.base", "util.treemap", "chart.brush.treemap.container" ], function(_, util, Container) {
    ...
    // normalize - the Bruls algorithm assumes we're passing in areas that
	// nicely fit into our
    // container box, this method takes our raw data and normalizes the data
	// values into
    ...
    // treemapMultidimensional - takes multidimensional data (aka
	// [[23,11],[11,32]] - nested array)
    ...
    function treemapMultidimensional(data, width, height, xoffset, yoffset) {
        ...
        if(_.typeCheck("array", data[0])) { // if we've got more dimensions of
											// depth
            for(var i = 0; i < data.length; i++) {
    ...
    // flattenTreemap - squarify implementation returns an array of arrays of
	// coordinates
    ...
    // improveRatio - implements the worse calculation and comparision as given
	// in Bruls
    ...
    // sumMultidimensionalArray - sums the values in a nested array (aka
	// [[0,1],[[2,3]]])
...
jui.define("chart.brush.treemap", [ "util.base", "chart.brush.treemap.calculator", "chart.brush.treemap.nodemanager" ],
    ...
    TreemapBrush.setup = function() {
        return {
            /**
			 * @cfg {"top"/"center"/"bottom" } [orient="top"] Determines the
			 *      side on which the tool tip is displayed (top, center,
			 *      bottom).
			 */
            textOrient: "top", // or bottom
            /**
			 * @cfg {"start"/"middle"/"end" } [align="center"] Aligns the title
...
jui.define("chart.brush.canvas.line.tooltip", [], function() {
	 ...
	 *
	 * @param {String}
	 *            text Sets the Value of tooltip
	 * @param {Number}
	 *            x Sets the X coordinate of tooltip
	 * @param {Number}
	 *            y Sets the Y coordinate of tooltip
	 */
...
jui.define("chart.brush.canvas.line.pathpoint", ["chart.brush.canvas.line.tooltip"], function(Tooltip) {
	 ...
	 * @param {Number}
	 *            x Sets the X coordinate of point
	 * @param {Number}
	 *            y Sets the Y coordinate of point
	 * @param {Boolean}
	 *            isMax
	 * @param {Boolean}
	 *            isMin
...
jui.define("chart.widget.core", [ "util.base" ], function(_) {
    ...
    /**
	 * @class chart.widget.core implements core widget
	 * @extends chart.draw
	 * @alias CoreWidget
	 * @requires util.base
	 * @requires jquery
    ...
    CoreWidget.setup = function() {
        ...
        /**
		 * @property {Number} index [Read Only] Index which shows the sequence
		 *           how a widget is drawn.
		 */

        return {
            /**
			 * @cfg {Boolean} [render=false] Determines whether a widget is to
			 *      be rendered.
			 */
            render: false,
            /**
...
jui.define("chart.widget.tooltip", [ "util.base", "util.color" ], function(_, ColorUtil) {
    ...
    TooltipWidget.setup = function() {
        return {
            /**
			 * @cfg {"bottom"/"top"/"left"/"right"} Determines the side on which
			 *      the tool tip is displayed (top, bottom, left, right).
			 */
            orient: "top",
            /** @cfg {Boolean} [anchor=true] Remove tooltip's anchor */
            anchor: true,
            /**
			 * @cfg {Boolean} [all=false] Determines whether to show all values
			 *      of row data.
			 */
            all: false,
            ...
            line: false,
            /**
			 * @cfg {Boolean} [flip=false] When I went out of the area,
			 *      reversing the tooltip.
			 */
            flip: false,
            /**
			 * @cfg {Function} [format=null] Sets the format of the value that
            ...
            format: null,
            /**
			 * @cfg {Number} [brush=0] Specifies a brush index for which a
			 *      widget is used.
			 */
            brush: 0
        };
...
jui.define("chart.widget.title", [], function() {
    ...
    TitleWidget.setup = function() {
        return {
            ...
            text: "",
            /**
			 * @cfg {Number} [dx=0] Moves the x coordinate by a set value from
			 *      the location where the chart is drawn.
			 */
            dx: 0,
            /**
			 * @cfg {Number} [dy=0] Moves the y coordinate by a set value from
			 *      the location where the chart is drawn.
			 */
            dy: 0,
...
jui.define("chart.widget.legend", [ "util.base" ], function(_) {
    ...
    /**
	 * @class chart.widget.legend implements legend widget
	 * @extends chart.widget.core
	 * @alias LegendWidget
	 * @requires util.base
	 *
    ...
    var LegendWidget = function(chart, axis, widget) {
		...
		this.getLegendIcon = function(brush) {
			...
			for(var i = 0; i < count; i++) {
                ...
                // 타겟 별 포맷 설정
                if(_.typeCheck("function", widget.format)) {
                    text = this.format(target, axis.data);
                }
    ...
    LegendWidget.setup = function() {
        return {
            /**
			 * @cfg {"bottom"/"top"/"left"/"right" } Sets the location where the
			 *      label is displayed (top, bottom).
			 */
            orient: "bottom",
            /**
			 * @cfg {"start"/"center"/"end" } Aligns the label (center, start,
			 *      end).
			 */
            align: "center", // or start, end
            /**
			 * @cfg {Boolean} [filter=false] Performs filtering so that only
			 *      label(s) selected by the brush can be shown.
			 */
            filter: false,
            ...
            colors: null,
            /**
			 * @cfg {Boolean} [brushSync=false] Applies all brushes equally when
			 *      using a filter function.
			 */
            brushSync: false,
            /**
			 * @cfg {Number/Array} [brush=0] Specifies a brush index for which a
			 *      widget is used.
			 */
            brush: 0,
            /**
			 * @cfg {Function} [format=null] Sets the format of the key that is
    ...
    /**
	 * @event legend_filter Event that occurs when the filter function of the
	 *        legend widget is activated. (real name ``` legend.filter ```)
	 * @param {String}
	 *            target The selected data field.
	 */
...
jui.define("chart.widget.cross", [ "util.base" ], function(_) {
    ...
    CrossWidget.setup = function() {
        return {
            ...
            /**
			 * @cfg {Function} [xFormat=null] Sets the format for the value on
			 *      the X axis shown on the tooltip.
			 */
            xFormat: null,
            /**
			 * @cfg {Function} [yFormat=null] Sets the format for the value on
			 *      the Y axis shown on the tooltip.
			 */
            yFormat: null
        };
    }
...
jui.define("chart.widget.topologyctrl", [ "util.base" ], function(_) {
    ...
    TopologyControlWidget.setup = function() {
        return {
            /**
			 * @cfg {Boolean} [move=false] Set to be moved to see the point of
			 *      view of the topology map.
			 */
            move: false,
            /**
			 * @cfg {Boolean} [zoom=false] Set the zoom-in / zoom-out features
			 *      of the topology map.
			 */
            zoom: false,
            /**
...
jui.define("chart.widget.map.control", [ "util.base" ], function(_) {
    ...
    MapControlWidget.setup = function() {
        return {
            /**
			 * @cfg {"top"/"bottom" } Sets the location where the label is
