	...
	<div class="col-md-12 portlets">
		...
		<div class="panel">
			...
			<div class="panel-content">
				<form name="form01" id="form01" action="/statistics/transaction/hour/rnd" method="post">
				<div class="row pull-right">
					...
					<div class="col-md-1" style="width:80px;padding-left:5px;padding-right:5px;">
						...
						<select class="form-control" name="day" id="day">
							<c:forEach var="item" begin="1" end="${lastDay}" step="1">
								<option value="${item }" <c:if test="${day eq item}">selected</c:if>>${item}</option>
							</c:forEach>
						</select>
					</div>
				...
				<ul class="nav nav-tabs">
					...
					<li class=""><a href="#tab2" data-toggle="tab">정형인터페이스별</a></li>
					<li class=""><a href="#tab3" data-toggle="tab">비정형인터페이스별</a></li>
				</ul>
				<div class="tab-content" style="overflow-y: auto;">
								...
								<table class="table table-hover">
									...
									<tbody>
										...
										<tr>
											...
											<td><div style="width:60px;">분(min)</div></td>
										</tr>
										<tr>
											<td>건</td>
										</tr>
							...
							<div class="col-md-1" style="overflow-y:auto;width:86%;padding:0px 0px; ">
								<table class="table table-hover" style="width:1800px;">
									...
									<tbody>
										...
										<tr>
											<c:forEach var="item" items="${list}">
												<td style="text-align:right">${item.peakTrnscMin}</td>
											</c:forEach>
										</tr>
										<tr>
											<c:forEach var="item" items="${list}">
										...
										<tr>
											<c:forEach var="item" items="${list}">
												<td style="text-align:right;width:75px;">${item.peakDataMin}</td>
											</c:forEach>
										</tr>
										<tr>
											<c:forEach var="item" items="${list}">
					...
					<div class="tab-pane fade" id="tab2">
						<div class="col-md-1" style="width:18%;padding:0px 0px; ">
							<table class="table table-hover">
								<thead>
									<tr>
								...
								<tr>
									<td><div style="width:200px;word-break: break-all;">${ifMap.get(item.key)}<br>(${item.key})</div></td>
									<td>
									<div style="width:80px;">
										<div class="valueItem_c">전체</div>
										<div class="valueItem_c">성공</div>
										<div class="valueItem_c">실패</div>
										<div class="valueItem_c">처리량(MB)</div>
										<div class="valueItem_c">row수</div>
									</div>
									</td>
								</tr>
								</c:forEach>
						...
						<div class="col-md-1" style="overflow-y:auto;width:81%;padding:0px 0px; ">
							<table class="table table-hover">
								<thead>
									<tr>
									    <c:forEach begin="0" end="23" var="idx">
					...
					<div class="tab-pane fade" id="tab3">
						<div class="col-md-1" style="width:18%;padding:0px 0px; ">
							<table class="table table-hover">
								...
								<tbody>
								<c:forEach var="item" items="${iotList}">
								<tr>
									<td><div style="width:200px;word-break: break-all;">${eseIfMap.get(item.key)}<br>(${item.key})</div></td>
									<td>
									<div style="width:80px;">
										<div class="valueItem_c">전체</div>
										<div class="valueItem_c">성공</div>
...
function excelDownload() {
	var type= $($('.nav.nav-tabs li')[0]).hasClass('active') ? '1' : $($('.nav.nav-tabs li')[1]).hasClass('active')  ? '4': '14';
	var year = $('#year').val();
	var month = $('#month').val();
	var day = $('#day').val();
